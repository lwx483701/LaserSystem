...
//chiu 070815
//增加struct Range和构成RangeV，用于存储范围
//
struct Range
{
	Range(long lMin = 0,long lMax = 0,long lCol = -1)
	{
		m_lMin = lMin;
		m_lMax = lMax;
		m_lCol = lCol;
...
typedef std::vector<Range> RangeV;
...
public:
	//chiu 070815
	Range & GetRange(long lCol);
	void AddRange(long lMin,long lMax,long lCol);
	//creates a root
