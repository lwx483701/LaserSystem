	...
	if(OnItemLButtonDown(ht))
	{
		...
		{		
			...
			if(nIndex!=-1)
			{
				...
				if(IsRoot(pSelItem))//chiu 070814禁止在RootItem中使用Edit
					return;
			}
		}
	}
...
CEdit* CSuperGridCtrl::EditLabelEx(int nItem, int nCol)
{
	...
	CSmartEdit *pEdit = new CListEditCtrl(nItem, nCol, GetItemText(nItem, nCol));
	pEdit->Create(dwStyle, rect, this, IDC_EDITCTRL);

	//chiu 070816
	long lCol = GetRange(nCol).m_lCol;//如果找不到，返回值是-1,如果找到，nCol == lCol;
	if(lCol >= 0)
	{
		long lMin = GetRange(nCol).m_lMin;
		long lMax = GetRange(nCol).m_lMax;

		pEdit->SetParseType(SES_NUMBERS | SES_SIGNED);
		pEdit->SetRange(lMin,lMax);//chiu 070815
	}
...
void CSuperGridCtrl::OnControlLButtonDown(UINT nFlags, CPoint point, LVHITTESTINFO& ht)
{
	if(ht.iSubItem==0)
	{
		...
		DWORD dwStyle = WS_BORDER | WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL|ES_LEFT;
		CSmartEdit *pEdit = new CListEditCtrl(ht.iItem, ht.iSubItem, GetItemText(ht.iItem, ht.iSubItem));
		pEdit->Create(dwStyle, rcItem, this, 0x1233);	
	}
	else
...
UINT CSuperGridCtrl::GetCount(int iIndex) 
{
	ASSERT(iIndex<=GetRootCount());
	UINT nCount=0;
	int tempIndex=0;
	POSITION pos = m_RootItems.GetHeadPosition();
	while(pos!=NULL)
	{
		...
		{
			UINT _nCount=0;
			nCount = _GetCount(pRoot, _nCount);
			return nCount;
		}
		tempIndex++;
	}
...
//chiu 070815 Add
void CSuperGridCtrl::AddRange(long lMin,long lMax,long lCol)
{
	m_RangeV.push_back(Range(lMin,lMax,lCol));
}

Range & CSuperGridCtrl::GetRange(long lCol)
{
	if(!m_RangeV.empty())
	{
		for(int i = 0;i<m_RangeV.size();++i)
		{
			if(m_RangeV[i].m_lCol == lCol)
				return m_RangeV[i];
		}
	}
	return Range(0,0,-1);
