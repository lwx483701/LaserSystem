...
m_iNumColumns = 0;
	...
	for( ;; )
	{
		...
		if( InsertColumn( m_iNumColumns++, strHeading, LVCFMT_CENTER, iWidth ) == -1 )	//LVCFMT_LEFT
			return FALSE;
	}
...
int CSortListCtrl::AddItem( CString pszText[],int StrArraySize )
{
	...
	const int iIndex = InsertItem( GetItemCount(), pszText[0] );	
	
//	arrpsz[ 0 ] = new TCHAR[ lstrlen( pszText[0] ) + 1 ];
//	clrText[ 0 ] = crWindowText;
//	clrBak[ 0 ] = crWindow;
//	(void)lstrcpy( arrpsz[ 0 ], pszText[0] );
	
	bool NG=false;	
	for( int iColumn0 = 0; iColumn0 < m_iNumColumns; iColumn0++ )
	{
		
		VERIFY( CListCtrl::SetItem( iIndex, iColumn0, LVIF_TEXT, pszText[iColumn0], 0, 0, 0, 0 ) );
		
//		if ("NG"==pszText[iColumn0])
		if (("NG"==pszText[iColumn0])||(" "==pszText[iColumn0]))						//20170410
			NG=true;
	}
	
	for( int iColumn = 0; iColumn < m_iNumColumns; iColumn++ )
	{		
		VERIFY( CListCtrl::SetItem( iIndex, iColumn, LVIF_TEXT, pszText[iColumn], 0, 0, 0, 0 ) );
		
		arrpsz[ iColumn ] = new TCHAR[ lstrlen( pszText[iColumn] ) + 1 ];
		clrText[ iColumn ] = crWindowText;
//		clrBak[ iColumn ] = crWindow;
		if (NG)
			clrBak[ iColumn ] = RGB(255,255,0);
		else
			clrBak[ iColumn ] = crWindow;
		
		(void)lstrcpy( arrpsz[ iColumn ], pszText[iColumn] );
	}
...
void CSortListCtrl::SetBakColor(COLORREF crBak)
{
	this->crWindow=crBak;
}
