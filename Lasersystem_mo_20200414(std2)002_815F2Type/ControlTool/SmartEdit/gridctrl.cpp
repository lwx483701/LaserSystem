...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	m_lEditMax  = pow(2,30)-1;
	m_lEditMin  = -pow(2,30)+1;
...
void CGridCtrl::OnEditCell(int nRow, int nCol, UINT nChar)
{
    ...
    CreateInPlaceEditControl(rect, dwStyle, m_nEditType,IDC_INPLACE_CONTROL, 
                             nRow, nCol, Item.szText, nChar);
}

void CGridCtrl::CreateInPlaceEditControl(CRect& rect, DWORD dwStyle,WORD type, UINT nID,
                                         int nRow, int nCol,
                                         LPCTSTR szText, int nChar)
{
    // InPlaceEdit auto-deletes itself
   new CInPlaceEdit(this, rect, dwStyle, type, nID, nRow, nCol,m_lEditMax,m_lEditMin, szText, nChar);
// new CSmartEdit(this, rect, dwStyle, type, nID, nRow, nCol, szText, nChar);
}
...
void CGridCtrl::SetEditParseType( WORD type)
{
    m_nEditType = type;
}

void CGridCtrl::SetEditRange(long lMax,long lMin)
{
   ASSERT(lMax>lMin);
