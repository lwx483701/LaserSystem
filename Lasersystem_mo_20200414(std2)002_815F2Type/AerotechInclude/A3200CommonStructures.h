...
typedef enum
{
	...
	HOMETYPE_AtCurrentPositionFeedback = 5,
...
typedef enum
{
	...
	POSITIONFEEDBACKTYPE_ParallelLaserInterface = 7,
	/// \brief Serial Encoder
	POSITIONFEEDBACKTYPE_SerialEncoder = 8,
	/// \brief Resolute Absolute Encoder
	POSITIONFEEDBACKTYPE_ResoluteAbsoluteEncoder = 9,
	/// \brief Position Detector
	POSITIONFEEDBACKTYPE_PositionDetector = 10,
...
typedef enum
{
	...
	/// \brief Parallel Laser Interface
	VELOCITYFEEDBACKTYPE_ParallelLaserInterface = 7,
	/// \brief Serial Encoder
	VELOCITYFEEDBACKTYPE_SerialEncoder = 8,
...
{
	/// \brief This task is running in queue mode.
	QUEUESTATUS_QueueModeActive = (1u),
...
typedef enum
{
	...
	DATACOLLECTIONFLAGS_SampleTrigger = (1u << 5),
...
typedef enum
{
	...
	/// \brief Sample trigger mode is active.
	DATACOLLECTIONFLAGSBITS_SampleTriggerBit = 5,
	/// \brief Continuous data collection mode is active
...
/// \brief This value provides status information about the Resolute encoder interface. The following table describes each bit of this value.
typedef enum
{
	/// \brief 
	RESOLUTESTATUS_CRCError = (1u),
	/// \brief 
	RESOLUTESTATUS_EncoderWarning = (1u << 1),
	/// \brief 
	RESOLUTESTATUS_EncoderError = (1u << 2),
}	RESOLUTESTATUS;
typedef enum
{
	/// \brief 
	RESOLUTESTATUSBITS_CRCErrorBit = 0,
	/// \brief 
	RESOLUTESTATUSBITS_EncoderWarningBit = 1,
	/// \brief 
	RESOLUTESTATUSBITS_EncoderErrorBit = 2,
}	RESOLUTESTATUSBITS;
...
typedef enum
{
	...
	LOOPTRANSMISSIONMODE_Multisine = 5,
	/// \brief Uses a Multisine disturbance and excites both axes of a gantry
	LOOPTRANSMISSIONMODE_MultisineGantry = 6,
...
typedef enum
{
	...
	FLASHCONFIGSTATUS_Supported = (1u),
...
typedef enum
{
	...
	AXISSTATUS_ThermoCompEnabled = (1u << 27),
}	AXISSTATUS;
typedef enum
{
	/// \brief The axis is homed.
	...
	/// \brief Infinite Field of View (IFOV) is enabled for this axis. Enable IFOV by issuing the IFOV ON command. Disable IFOV by issuing the IFOV OFF command.
	AXISSTATUSBITS_IFOVEnabledBit = 12,
	/// \brief A physical drive is associated with this axis. Axes with no drive attached will clear this bit and operate as virtual axes.
	...
	/// \brief The ThermoComp feature is currently turned on.
	AXISSTATUSBITS_ThermoCompEnabledBit = 27,
}	AXISSTATUSBITS;
...
typedef enum
{
	...
	AXISFAULT_PositionErrorFault = (1u),
	...
	/// \brief The drive detected a problem with the feedback device specified by the PositionFeedbackType and PositionFeedbackChannel parameters. Or, the drive detected an invalid position feedback configuration.
	AXISFAULT_PositionFeedbackFault = (1u << 7),
	/// \brief The drive detected a problem with the feedback device specified by the VelocityFeedbackType and VelocityFeedbackChannel parameters. Or, the drive detected an invalid velocity feedback configuration.
	...
	/// \brief The amplifier temperature is not within the operating range. For a FireWire drive, this fault occurs at temperatures greater than 75° C. For a HyperWire drive, this fault occurs at temperatures less than 0° C or greater than 75° C.
	...
	AXISFAULT_MissedInterruptFault = (1u << 29),
...
typedef enum
{
	...
	/// \brief The drive failed to execute the control loop for this axis because a previous execution of the control loop exceeded the available processing time.
	AXISFAULTBITS_MissedInterruptFaultBit = 29,
	/// \brief The drive encountered an internal error that caused it to disable. Contact Aerotech Global Technical Support.
...
typedef enum
{
	...
	DRIVESTATUS_GalvoPowerCorrection = (1u << 12),
...
typedef enum
{
	...
	/// \brief Galvo Power Correction is configured on this axis.
	DRIVESTATUSBITS_GalvoPowerCorrectionBit = 12,
	/// \brief The drive detected that no motor supply voltage is present.
...
typedef enum
{
	...
	TASKSTATUS0_ProgramAssociated = (1u),
...
typedef enum
{
	...
	TASKSTATUS1_MotionModeAbsOffsets = (1u),
	...
	/// \brief This bit represents internal status.
	TASKSTATUS1_IFOVBufferHold = (1u << 25),
}	TASKSTATUS1;
typedef enum
{
	/// \brief Circular center points are specified in absolute coordinates.
	...
	/// \brief This bit represents internal status.
	TASKSTATUS1BITS_IFOVBufferHoldBit = 25,
}	TASKSTATUS1BITS;
...
typedef enum
{
	...
	DATACOLLECTIONSAMPLETRIGGER_OnAnyChange = 1,
	/// \brief Collect sample when the condition item increases.
	DATACOLLECTIONSAMPLETRIGGER_OnIncrease = 2,
	/// \brief Collect sample when the condition item decreases.
	DATACOLLECTIONSAMPLETRIGGER_OnDecrease = 3,
}	DATACOLLECTIONSAMPLETRIGGER;
...
typedef enum
{
	...
	/// \brief Channel 5
	POSITIONFEEDBACKCHANNEL_Channel5 = 5,
	/// \brief Channel 6
	POSITIONFEEDBACKCHANNEL_Channel6 = 6,
	/// \brief Channel 7
	POSITIONFEEDBACKCHANNEL_Channel7 = 7,
...
typedef enum
{
	...
	/// \brief Channel 5
	VELOCITYFEEDBACKCHANNEL_Channel5 = 5,
	/// \brief Channel 6
	VELOCITYFEEDBACKCHANNEL_Channel6 = 6,
	/// \brief Channel 7
	VELOCITYFEEDBACKCHANNEL_Channel7 = 7,
