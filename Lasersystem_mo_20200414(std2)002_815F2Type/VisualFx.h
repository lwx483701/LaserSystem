...
// Operations
public:
void SetOutward(CFont* pfont, COLORREF crColor, UINT nFormat);
...
public:
  ...
  TTabItem(CWnd *pParent, LPCTSTR label,DWORD dwID);
...
protected:
  ...
  TTabItem *addTab(CWnd *pWnd, LPCTSTR szLabel,DWORD dwID);
...
protected:
  ...
  // Create pane
  virtual TTabItem *CreatePane(LPCTSTR lpszLabel, DWORD dwID,CRuntimeClass *pViewClass, 
                                CCreateContext *pContext);
  // Create splitter
  virtual TTabItem *CreatePane(LPCTSTR lpszLabel,DWORD dwID, int nRows, int nCols, 
                                CWnd *pWnd, UINT nID = AFX_IDW_PANE_FIRST);

public:
  TVisualObject* GetObj(DWORD dwID)const;
  TTabWnd();
  virtual ~TTabWnd();
...
protected:
	CWnd* GetFocusWnd(TVisualObject* obj);
	DWORD GetFocusID(TVisualObject* obj);					//20170524 新增
  //{{AFX_MSG(TTabWnd)
...
private:
  ...
  static TVisualFramework *m_pFramework;   // Pointer to framework that owns this object
...
public:
  ...
  static TVisualFramework *GetFramework(void);
...
private:
  ...
  BOOL execCreateOurbar(CWnd* pWnd,TVisualObject* pObject);
