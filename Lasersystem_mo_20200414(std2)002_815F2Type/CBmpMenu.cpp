...
// CBmpMenu
// constructor
extern CMainRunView* pCMainRunView;									//20190215
...
GetSysColorBitmap(HDC hDC, HBITMAP hSourceBitmap, BOOL bMono, BOOL bSelected)
{
	...
	//  Create a memory DC for the source and destination bitmaps
	...
	//  Get the height and width of the source bitmap.
	...
	//  Select the source bitmap into the source DC. Create a
...
void MenuToolBar::OnPostLbuttonMsg(UINT nFlags, LPARAM lp)
{
	...
	{
	case ID_FUN_LASERWARNUP:
#ifndef IO_C
		GetControl().LaserWarmup(60);
#else
		{
			CWnd* pWnd = ((CBmpMenu*)GetParent())->m_pOwnerWnd;
			if (pWnd != NULL)
				::PostMessage(pWnd->m_hWnd,WM_MSG_PREHOT,NULL,NULL);
		}
#endif
		GetLog().Append_pro(10001,20001);//"激光上电","激光预热"
		break;
    case ID_FUN_TABLEOUT:
		...
		break;
	case ID_TABLESUCTIONON:
		GetControl().ChannelOutput(OFANPOWERON, true); 
		GetControl().ChannelOutput(OTABLEVACUUMVALVE1,IO_ON);
		GetControl().ChannelOutput(OTABLEVACUUMVALVE2,IO_ON);
		break;
	case ID_TABLESUCTIONOFF:
		GetControl().ChannelOutput(OFANPOWERON, false); 
...
*/
		...
		pCMainRunView->GetDlgItem(IDC_BT_RUN_START)->EnableWindow(FALSE);
		pCMainRunView->GetDlgItem(IDC_BT_HOMEALL)->EnableWindow(FALSE);
		pCMainRunView->GetDlgItem(IDC_BT_LASERWARMUP)->EnableWindow(FALSE);
...
//				AfxMessageBox("功率测试出现异常，请确认!");
		...
		GetControl().SetRunState(CControl::END);
		pCMainRunView->GetDlgItem(IDC_BT_RUN_START)->EnableWindow(TRUE);
		pCMainRunView->GetDlgItem(IDC_BT_HOMEALL)->EnableWindow(TRUE);
		pCMainRunView->GetDlgItem(IDC_BT_LASERWARMUP)->EnableWindow(TRUE);
		break;
	}	
}