...
//TransparentStatic.cpp://自定义透明背景静态文本ly add 20110902

/////////////////////////////////////////////////////////////////////////////
// CTransparentStatic

CTransparentStatic::CTransparentStatic()//ly add 20110902
{
	m_pfont   = new CFont;
	m_pfont->CreateFont										//20160907
		(
		15,					// nHeight
		0,					// nWidth
		0,					// nEscapement
...
TTabItem::TTabItem(CWnd *pParent, LPCTSTR szLabel)
{
  ...
  m_pCaption = new CTransparentStatic();
  ASSERT(m_pCaption);
...
TTabItem::TTabItem(CWnd *pParent, LPCTSTR szLabel,DWORD dwID)
{
  ...
  RECT rect;
  ::ZeroMemory(&rect,sizeof(RECT));
  m_pCaption = new CTransparentStatic();//CStatic;
  ASSERT(m_pCaption);
  m_pCaption->Create(szLabel, WS_CHILD|SS_CENTER|WS_VISIBLE,rect,pParent);
}
...
DWORD TTabItem::GetID(void) const
{
	return m_dwTabID;
}
...
//	m_Font.CreateFontIndirect(&metrics.lfStatusFont);
	m_Font.CreateFont(40,24,0,0,600,0,0,0,GB2312_CHARSET,OUT_CHARACTER_PRECIS,CLIP_DEFAULT_PRECIS,PROOF_QUALITY,0,"宋体");
	wdc.SelectObject(&m_Font);
}

// Add a tab to this window
TTabItem *TTabWnd::addTab(CWnd *pWnd, LPCTSTR szLabel,DWORD dwID)
{
  ASSERT(pWnd);
  ASSERT(szLabel);

  TTabItem *pItem = new TTabItem(this,szLabel,dwID);
  pItem->m_pWnd = pWnd;
...
int TTabWnd::drawSelTabTop(CDC *pDC, int x, CRect& client, TTabItem *pItem)
{
  ...
  pDC->FillSolidRect(x+3, y+3,textSize.cx+TAB_SPACE-6, TABSEL_HEIGHT, 
                  ::GetSysColor(COLOR_BTNFACE));//RGB(0,255,0)
...
int TTabWnd::drawSelTabBottom(CDC *pDC, int x, CRect& client, TTabItem *pItem)
{
  ...
  pDC->FillSolidRect(x+3,y,textSize.cx+TAB_SPACE-6, TABSEL_HEIGHT-2, 
                    RGB(0,255,0));//::GetSysColor(COLOR_BTNHIGHLIGHT)COLOR_BTNFACE//ly change 20110902
...
CWnd* TTabWnd::GetFocusWnd(TVisualObject *obj)
{
  TVisualObject *pObject;

  TVisualObjectList::iterator it;
  if(obj->m_ObjectList.size())
  {
     for (it = obj->m_ObjectList.begin(); it != obj->m_ObjectList.end(); it++)
	 {
         pObject = *it;
		 if(pObject->GetFocusFlag())
			 return pObject->m_pWnd;
	 }
	 pObject = *obj->m_ObjectList.begin();
	 return pObject->m_pWnd;
  }	
  else
  {
...
DWORD TTabWnd::GetFocusID(TVisualObject* obj)
{
  TVisualObject *pObject;
...
TVisualObject* TTabWnd::GetObj(DWORD dwID) const
{
  TVisualFramework *m_pwork=TVisualObject::GetFramework(); 

  return m_pwork->Get(dwID);
}
...
//-------------------------------------------------------------		//20170524
	DWORD oldPaneID=GetFocusID(GetObj(pOldPane->m_dwTabID));
	DWORD newPaneID=GetFocusID(GetObj(pNewPane->m_dwTabID));
	
	if((oldPaneID==MAINRUNVIEW)||(oldPaneID==IMAGEPARASETVIEW))
		GetFocusWnd(GetObj(pOldPane->m_dwTabID))->PostMessage(WM_UNDISPLAY, (UINT)0, (LONG)0);
	
	if(((oldPaneID==MAINRUNVIEW)&&(newPaneID==IMAGEPARASETVIEW))||((oldPaneID==IMAGEPARASETVIEW)&&(newPaneID==MAINRUNVIEW)))
		Delay(100);
	
	if((newPaneID==MAINRUNVIEW)||(newPaneID==IMAGEPARASETVIEW))
		GetFocusWnd(GetObj(pNewPane->m_dwTabID))->PostMessage(WM_DISPLAY, (UINT)0, (LONG)0);
//-------------------------------------------------------------
...
// Deactivate old pane
		if (m_nSelectedTab != -1) 
		{
			// pOldPane->m_pWnd->EnableWindow(FALSE);//yutingxun  06-12-04
			pOldPane->m_pWnd->ShowWindow(SW_HIDE);
			GetObj(pOldPane->m_dwTabID)->SetFocusFlag(FALSE);
		}
// Activate new pane
		pNewPane->m_pWnd->EnableWindow(TRUE);
		pNewPane->m_pWnd->ShowWindow(SW_SHOW);
		GetObj(pNewPane->m_dwTabID)->SetFocusFlag(TRUE);
		
		pNewPane->m_bFocus = TRUE;
...
{
  TVisualObject *obj; 
  static TTabItem *pOldPane;  
  int nNewTab = HitTest(point.x, point.y); 
  TTabItem *pNewPane = findTabItem(nNewTab);

  SetActivePane(nNewTab);

  if( pNewPane!=NULL && pNewPane->m_pWnd!=NULL )
  {
	  if( pOldPane!=NULL && pOldPane->m_pWnd!=NULL)
	  {
		switch(pOldPane->GetID())
		{
			case MAINRUNVIEW: 
			case IMAGEPARASETVIEW:
			case FILEINFOSETBASE:
//				GetFocusWnd(GetObj(pOldPane->m_dwTabID))->PostMessage(WM_UNDISPLAY,(UINT)0,(LONG)0);	//20170524	
				break;
			case MOTIONCONTROLTESTVIEW :
			case SIGNALINPUTVIEW: 
				pOldPane->m_pWnd->PostMessage(WM_INPUTUNCHECK, (UINT)0, (LONG)0);	
				break;
		}
	  }

	  int n =findTabItem(nNewTab)->GetID();

	  switch(pNewPane->GetID())
	  {
	    case FILEINFOSETBASE:
		   obj = GetObj(pNewPane->GetID());	  
//		   if(n = obj->m_ObjectList.size())										
//		   {
//		       GetFocusWnd(obj)->PostMessage(WM_DISPLAY, (UINT)0, (LONG)0);		//20170524
//		   }
		   break;
		case MAINTAINBASE:
		   obj = GetObj(pNewPane->GetID());	  
		   if(n = obj->m_ObjectList.size())
		   {
		       GetFocusWnd(obj)->PostMessage(WM_INPUTCHECK, (UINT)0, (LONG)0);
		   }
		   break;
	    case MAINRUNVIEW: 
		case IMAGEPARASETVIEW:
//		  pNewPane->m_pWnd->PostMessage(WM_DISPLAY, (UINT)0, (LONG)0);			//20170524
		  break;
		case TOOLPARAVIEW:
		  pNewPane->m_pWnd->PostMessage(WM_TOOLPARASHOW, (UINT)0, (LONG)0);	
   		  break;
		case MOTIONCONTROLTESTVIEW :
		case SIGNALINPUTVIEW: 
		  pNewPane->m_pWnd->PostMessage(WM_INPUTCHECK, (UINT)0, (LONG)0);	
		  break;
	  }
	  pOldPane = pNewPane;
  }
...
TTabItem *TTabWnd::CreatePane(LPCTSTR lpszLabel,DWORD dwID, CRuntimeClass *pViewClass, 
                                 CCreateContext *pContext)
{
  ...
  TTabItem *pTab = addTab(pWnd,lpszLabel,dwID);
  ASSERT(pTab);
  if (m_TabList.size() != 1) {
...
TTabItem *TTabWnd::CreatePane(LPCTSTR lpszLabel, DWORD dwID,int nRows, int nCols, 
                                 CWnd *pWnd, UINT nID)
{
  ASSERT(pWnd);
...
// Private constructor
TVisualObject::TVisualObject():  m_bFocus(FALSE)
{
}

TVisualFramework * TVisualObject::m_pFramework=NULL;
// Create a plain view
                       ...
                       CRuntimeClass *pClass, CSize size, DWORD dwStyle)
{
  ...
  ASSERT(pClass);
  ASSERT(pClass->IsDerivedFrom(RUNTIME_CLASS(CView)) ||
         pClass->IsDerivedFrom(RUNTIME_CLASS(CWnd)));//(TTabWnd)

  zeroAll();
  m_dwId = dwId;
  if(pClass->IsDerivedFrom(RUNTIME_CLASS(CGfxOutBarCtrl)))
    m_nObjectType = OT_OUTBAR;
  else
...
void TVisualObject::SetFocusFlag(BOOL bFocus)
{
    m_bFocus = bFocus;
}

BOOL TVisualObject::GetFocusFlag() const
{
    return m_bFocus;
}
...
BOOL TVisualObject::CanFocus(void)
{
  ...
  if (m_pWnd->IsKindOf(RUNTIME_CLASS(CSplitterWnd)) ||
	  ...
	  m_pWnd->IsKindOf(RUNTIME_CLASS(CGfxOutBarCtrl)))
  {
    return FALSE;
  }
  return TRUE;
  ...
  if (pOwner->m_nObjectType == TVisualObject::OT_TAB) {
    ...
    if ((pObject->m_nObjectType != TVisualObject::OT_SPLITTERVIEW) && 
        ((pObject->m_nObjectType != TVisualObject::OT_SPLITTERSPLITTER)&&
		 pObject->m_nObjectType != TVisualObject::OT_OUTBAR))
    {
      ASSERT(FALSE);
    }
...
void TVisualFramework::Destroy(void)
{
  ...
  TVisualObjectList::iterator it;
  TVisualObjectList::iterator chit;

#if 1
  for (it = m_ObjectList.begin(); it != m_ObjectList.end(); it++) {
    pObject = *it;
//	for(chit = pObject->m_ObjectList.begin();chit != pObject->m_ObjectList.end(); chit++) {
//	pObject = *chit;
    execDestroy(pObject);
//	}
  }
#else
  for (it = m_ObjectList.end(); it != m_ObjectList.begin(); it--) {
      it--;
	  pObject = *it;
    execDestroy(pObject);
  }
...
#else
	  for (it = pObject->m_ObjectList.end(); it != pObject->m_ObjectList.begin(); it--) {
       pObj = *it;
        execDestroy(pObj);
	  }
#endif
...
BOOL TVisualFramework::execCreate(CWnd *pWnd, TVisualObject *pObject)
{
  ...
  case (TVisualObject::OT_VIEW):
    ...
    break;
  case (TVisualObject::OT_OUTBAR):
    rc = execCreateOurbar(pWnd, pObject);
	break;
  }
  return rc;
}
...
BOOL TVisualFramework::execCreateTabView(CWnd *pWnd, TVisualObject *pObject)
{
  ...
  TTabWnd *pTab = (TTabWnd*)pWnd;
  TTabItem *pItem;
  pItem = pTab->CreatePane(pObject->m_strTitle,pObject->m_dwId, pObject->m_pRuntimeClass, 
                            pObject->m_pContext);
  ASSERT(pItem);
...
BOOL TVisualFramework::execCreateSplitter(CWnd *pWnd, TVisualObject *pObject)
{
  ...
  if (pWnd->IsKindOf(RUNTIME_CLASS(TTabWnd))) {
    TTabWnd *pTab = (TTabWnd*)pWnd;
    TTabItem *pItem = pTab->CreatePane(pObject->m_strTitle,pObject->m_dwId, pObject->m_nRows, 
...
BOOL TVisualFramework::execCreateTabWnd(CWnd *pWnd, TVisualObject *pObject)
{
  ...
  if (pWnd->IsKindOf(RUNTIME_CLASS(TTabWnd))) {
    TTabWnd *pTab = (TTabWnd*)pWnd;
    TTabItem *pItem = pTab->CreatePane(pObject->m_strTitle,pObject->m_dwId, pObject->m_pRuntimeClass,
                                       pObject->m_pContext);
    ASSERT(pItem);
    pObject->m_pWnd = pItem->GetSafeWnd();
    ASSERT(pObject->m_pWnd);
...
// Create Outbar
BOOL TVisualFramework::execCreateOurbar(CWnd *pWnd, TVisualObject *pObject)
{
  ASSERT(pWnd);
  ASSERT(pWnd->IsKindOf(RUNTIME_CLASS(CSplitterWnd)));
  ASSERT(pObject);
  ...
  pObject->m_pWnd =(CWnd*)pSplitter->GetPane(pObject->m_nRowIndex, pObject->m_nColIndex);
  ASSERT(pObject->m_pWnd);
  ASSERT(::IsWindow(pObject->m_pWnd->m_hWnd));
  pObject->m_pParent = pSplitter; 
...
void TVisualFramework::setTabWndProperties(TVisualObject *pObject)
{
  ...
  if (pObject->m_pWnd->IsKindOf(RUNTIME_CLASS(TTabWnd))) {
    ...
    if (pObject->m_dwStyle & TVisualObject::TOS_SELECTED)//TOS_TABTOP
